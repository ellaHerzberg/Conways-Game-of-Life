...
class Button:
    def __init__(self, surface, x, y, width, height, state='', id='',function=0, colour=(255,255,255), hover=(240,240,240), border=True, border_width=2, border_colour=(0,0,0), text='', font_name ='ariel', text_size=20, text_colour=(0,0,0), bold_text=False):  
        self.type = 'button'
        self.x = x
        self.y = y
        self.pos = vec(x,y)
    ...
    def draw(self):
        if self.showing:
            if self.border:
                ...
                if self.hovered:
                    pygame.draw.rect(self.image, self.hover, (self.border_width, self.border_width, self.width-(self.border_width*2), self.height-(self.border_width*2)))
                else:
